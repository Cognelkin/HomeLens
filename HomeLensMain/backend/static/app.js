...
//const stylesEl = document.getElementById('styles'); // NEW
//
...
//      li.innerHTML = `<a href="${rec.url}" target="_blank">${rec.title}</a> - ${rec.price} ${rec.currency || ''}`;
//      recEl.appendChild(li);
//    }
//  } else {
//    recEl.textContent = "(no recommendations)";
//  }
//}
//
//function renderDetections(data) {
...
//      const li = document.createElement('li');
//      li.textContent = `${s.name} (${(s.confidence*100).toFixed(1)}%)`;
//      stylesEl.appendChild(li);
//    }
//  } else {
//    const li = document.createElement('li');
//    li.textContent = '(no styles detected)';
//    stylesEl.appendChild(li);
//  }
//}
...
function renderDetections(data) {
  ...
  if (data.recommendations && data.recommendations.length > 0) {
    for (const item of data.recommendations) {
      ...
      const price = item.price?.value ? `$${item.price.value}` : "Price N/A";
      const url = item.itemWebUrl || "#";
      li.innerHTML = `<a href="${url}" target="_blank">${title} - ${price}</a>`;
      recEl.appendChild(li);
    }
  } else {
    const li = document.createElement('li');
